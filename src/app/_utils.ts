import { FormElementField } from "./components/form-elements/form-elements.component";

const materialImportsNeeded = [
  {module: 'MatFormFieldModule', path: 'form-field'},
  {module: 'MatIconModule', path: 'icon'},
  {module: 'MatInputModule', path: 'input'},
  {module: 'MatSelectModule', path: 'select'},
  {module: 'MatDatepickerModule', path: 'datepicker'},
];

  export function fullComponentCode2(formCanvas: FormElementField[][]): string {
    const materialImportsStatement = materialImportsNeeded.map(({module, path}) => 
        `import { ${module} } from '@angular/material/${path}';`
    ).join('\n');
...
_filteredFormCanvas = signal(${JSON.stringify(formCanvas, null, 5)});
